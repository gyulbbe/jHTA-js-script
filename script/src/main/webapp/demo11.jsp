...
<script type="text/javascript">
	...
	/*
		전역변수(글로블변수, 외부변수)
			+ 함수의 바깥에서 정의된 함수다.
			+ 모든 함수에서 접근할 수 있다.
			+ 일반적으로 프로젝트에서 번역변수는 아예 사용하지 않거나 최소한으로 사용한다.
			  단, 프로젝트 전체에서 광범위하게 사용되는 값은 전역변수로 관리할 수 있고, 주로 상수로 정의한다.
			  
		지역변수
			+ 함수 내부에서 정의된 변수다.
			+ 해당 함수내에서만 사용할 수 있다.
			+ 함수 내에서 변수를 정의할 때 let 키워드를 생략하고 함수를 정의하며 해당 변수는 무조건 전역변수가 된다.
		...
		매개변수
			...
			*  함수 호출시 인자값을 전달받지 못한 매개변수는 undefined값을 가진다.
			...
			* 인자값을 전달받은 매개변수는 인자값이 매개변수에 대입되고,
			  인자값을 전달받지 못한 매개변수 기본값이 매개변수에 대입된다.
		*/
	...
	/*
		반환값
			...
			* 함수가 값을 반환하지 않으면 undefined값을 반환한다.
